              ...
              <Link className="ph-card" key={d.slug} href={`/desafios/${d.slug}`}>
                <div className="body" style={{ padding: '16px 18px' }}>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: 8 }}>
                    <span style={{ fontSize: 11.5, fontWeight: 800, color: PV.white, background: NIVEL_COR[d.nivel] || 'var(--clay)', padding: '4px 10px', borderRadius: 100, textTransform: 'uppercase', letterSpacing: '.04em' }}>{d.nivel}</span>
            ...
            <ol style={{ margin: 0, paddingLeft: 0, listStyle: 'none', display: 'grid', gap: 12 }}>
              {[
                ...
                <li key={t} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <span style={{ display: 'grid', placeItems: 'center', width: 34, height: 34, borderRadius: '50%', background: 'var(--clay)', color: PV.white, fontWeight: 800, flexShrink: 0 }}>{i + 1}</span>
                  <div>
