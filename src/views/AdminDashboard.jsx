    ...
    <div style={{ paddingBottom:'100px' }}>
      <div className="ig-pagehead">
        <div>
          ...
          <h1>Avançado</h1>
          <p>GPS ao vivo, trechos, expedições, ranking, mapa, parceiros e configurações.</p>
        </div>
      </div>

      {/* Tabs IGNIS */}
      <div className="ig-tabs" style={{ overflow:'auto', flexWrap:'nowrap', scrollbarWidth:'none' }}>
        {TABS.map(tab => (
          <button key={tab.id} className={`ig-tab${activeTab === tab.id ? ' active' : ''}`} style={{ flexShrink:0 }} onClick={() => setActiveTab(tab.id)}>
            {tab.icon} {tab.label}
          </button>
        ))}
      </div>
